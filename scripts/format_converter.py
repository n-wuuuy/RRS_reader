...
class Converter:
    ...
    def __init__(self, data_news: Dict,
                 path_html: str = None, path_pdf: str = None) -> None:
        """Initialze Converter class."""
    ...
    def convert_news(self):
        """Converts data to html and pdf formats."""
        try:
    ...
    def create_image_folder(self, save_path: str) -> None:
        """Creates a folder for pictures."""
        self.img_folder = os.path.join(os.path.dirname(save_path),
    ...
    def download_image(self, link: str, img_number: int) -> str:
        """Downloads pictures and saves them to a folder."""
        image_path = os.path.join(self.img_folder,
                                  ''.join(f'image_link{img_number}.jpg'))
    ...
    def create_html(self):
        ...
        def add_main_part():
            """Adds the main data to the html string, after converting it to html format."""
        ...
        def add_explanatory_part():
            """Adds additional data to the html string by first converting it to html format."""
            ...
            if (single_date.get('Image') != 'no information'):
                image_path = self.download_image(single_date.get('Image'),
                                                 img_number)
                if not image_path:
                    return
                doc.stag('br')
        ...
        with tag('html'):
            with tag('body'):
                with tag('p'):
                    for img_number, single_date in enumerate(self.data_news,
                                                             start=1):
                        with tag('b'):
                            add_main_part()
    ...
    def conver_to_html(self, source_html: str):
        """Puts data and saves it in html format."""
        try:
    ...
    def conver_to_pdf(self, source_html: str):
        """Puts data and saves it in pdf format."""
        try:
